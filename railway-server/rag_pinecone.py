"""
Production-ready RAG pipeline using Groq + Pinecone + Jina AI Embeddings.

Features
- Serverless Pinecone index bootstrap
- Batch upsert of vectors with metadata
- Embedding generation via Jina AI API (no local models)
- Semantic retrieval
...
JINA_EMBED_BATCH_SIZE = int(os.getenv("JINA_EMBED_BATCH_SIZE", "32"))
...
def _embed_texts_jina(
	...
	"""Call Jina AI embeddings API in batches."""
	headers = {
	...
	for batch_start in range(0, len(texts), JINA_EMBED_BATCH_SIZE):
		batch = texts[batch_start : batch_start + JINA_EMBED_BATCH_SIZE]
		payload: Dict[str, Any] = {"model": model, "input": batch}

		for attempt in range(max_retries + 1):
			try:
				with httpx.Client(timeout=60.0) as client:
					resp = client.post(JINA_API_URL, json=payload, headers=headers)
				...
				if attempt >= max_retries:
					raise RuntimeError(f"Jina AI embedding failed after {max_retries} retries: {exc}") from exc
				sleep_s = retry_base_seconds * (2 ** attempt)
				logging.warning("Jina embedding retry %s/%s in %.1fs: %s", attempt + 1, max_retries, sleep_s, exc)
				time.sleep(sleep_s)
...
def embed_query(api_key: str, query: str, model: str = EMBEDDING_MODEL) -> List[float]:
	return embed_texts(api_key, [query], model=model)[0]


def build_embedding_client(model: str = EMBEDDING_MODEL) -> str:
	"""Returns the Jina AI API key used for embeddings."""
...
def ensure_index(pc: Pinecone, index_name: str = INDEX_NAME, dimension: int = 1024) -> None:
...
def ingest_chunks(
	...
	api_key = build_embedding_client(embedding_model)
	pc = build_pinecone_client()

	records = load_chunk_records(input_path, chunk_source=chunk_source)
	probe_vec = embed_texts(api_key, [records[0].content], model=embedding_model)[0]
	...
	for batch in _iter_batches(records, UPSERT_BATCH_SIZE):
		...
		embeddings = embed_texts(api_key, texts, model=embedding_model)
...
def semantic_query(
	...
	api_key = build_embedding_client(embedding_model)
	qvec = embed_query(api_key, query, model=embedding_model)
...
def warmup_runtime(
	...
	api_key = build_embedding_client(embedding_model)
	probe = embed_query(api_key, "warmup", model=embedding_model)
...
def build_cli() -> argparse.ArgumentParser:
	parser = argparse.ArgumentParser(description="Production RAG with Groq + Pinecone + Jina AI")
