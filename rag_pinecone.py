...
def is_non_kb_query(query: str) -> bool:
	...
	if not q:
		return True
	return bool(NON_KB_QUERY_RE.search(q))
...
def embed_query(api_client: Optional[OpenAI], query: str, model: str = EMBEDDING_MODEL) -> List[float]:
...
def ingest_chunks(input_path: str, namespace: str = "default", chunk_source: str = "base_chunks") -> None:
	api_client = None if is_sentence_transformer_model(EMBEDDING_MODEL) else build_groq_client()
	pc = build_pinecone_client()
...
def rag_answer(
	...
	matches = prefetched_matches if prefetched_matches is not None else semantic_query(query=user_query, top_k=top_k, namespace=namespace)
